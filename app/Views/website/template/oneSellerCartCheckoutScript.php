<script>
    ...
    function selectAddressType(addressType) {
        ...
        const locationDivs = document.querySelectorAll('.flex.space-x-4 <?= flex_direction() ?> > div');
    ...
    function selectDeliveryMethod(method) {
        ...
        allCards.forEach(card => {
            card.classList.remove('bg-green-100', 'border-green-700', 'text-green-700'); // Remove active styles
        ...
        selectedCard.classList.add('bg-green-100', 'border-green-700', 'text-green-700'); // Add active styles
    ...
    function loadDeliveryMethod() {
        ...
        if (storedMethod) {
            ...
            selectedCard.classList.add('bg-green-100', 'border-green-700', 'text-green-700'); // Active styles for the selected card
            selectedCard.classList.remove('bg-white'); // Remove default styles
    ...
    function setActiveTime(button) {
        ...
        allButtons.forEach(btn => btn.classList.remove('bg-green-100', 'border-green-500', 'text-green-700')); // Remove previously active styling

        // Add 'active' class to the clicked button
        button.classList.add('bg-green-100', 'border-green-500', 'text-green-700'); // Add active styles
                ...
                result.data.forEach(coupon => {
                    ...
                    if (appliedCoupon && +appliedCoupon.coupon_id === +coupon.id) {
                        ...
                        if (couponCardElement) {
                            couponCardElement.classList.add('bg-green-100', 'border', 'border-green-500'); // Applied styles
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (couponCard) {
                    ...
                    couponCard.classList.add('bg-green-100', 'border', 'border-green-500'); // Applied styles   
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // // Elements to update
        // const couponAmountElements = document.getElementsByClassName('couponAmount');
        // const grandTotalElements = document.getElementsByClassName('grand_total');
        // const walletAppliedElements = document.getElementsByClassName('wallet_applied');
        // const remainingWalletBalanceElements = document.getElementsByClassName('remaining_wallet_balance');

        // let additional_charge = 0;

        // if (document.getElementsByClassName('additional_charge_div').length > 0) {
        //     const additionalChargeElement = document.getElementsByClassName('additional_charge')[0]; // Get the first matching element
        //     if (additionalChargeElement) {
        //         const chargeText = additionalChargeElement.textContent.trim(); // Extract and clean the text
        //         additional_charge = parseFloat(chargeText) || 0; // Parse the text as a float and ensure a fallback to 0
        //     }
        // }
        ...
        // // Base amounts (Replace with your actual PHP variables or defaults)
        // const subtotal = <?= esc($subtotal) ?> || 0;
        // const taxTotal = <?= esc($taxTotal) ?> || 0;
        // const deliveryCharge = <?= esc($deliveryCharge) ?> || 0;

        // // Calculate the grand total
        // const grandTotal = subtotal + taxTotal + deliveryCharge + additional_charge - couponAmount - walletApplied;
        ...
        // // Update grand total
        // Array.from(grandTotalElements).forEach(element => {
        //     element.innerText = grandTotal > 0 ? grandTotal.toFixed(2) : 0; // Ensure positive total
        // });
                ...
                if (paymentMethode == 5) {
                    ...
                    ?>
                            ...
                            var elements = stripe.elements({
                                // clientSecret: '<?= $paymentMethod['secret_key'] ?>',
                                mode: 'payment',
                                currency: 'usd',
                                amount: 1099,
                            });
