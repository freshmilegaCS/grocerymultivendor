...
class Product extends BaseController
{
    public function index()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            $categories = $categoryModel->getCategoriesForSeller();
            ...
            return view('sellerPanel/product/add', [
                'settings' => $appSetting,
    ...
    public function view()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            return view('sellerPanel/product/list', [
                'settings' => $appSetting,
                'categories' => $categories
            ]);
        } else {
    ...
    public function list()
    {
        ...
        foreach ($products as $row) {
            foreach ($row['variants'] as $variants) {
                ...
                $action = "<a data-tooltip='tooltip' title='Edit Product' href='" . base_url("seller/product/edit/{$row['id']}") . "' class='btn btn-primary-light  btn-xs'>
    ...
    public function delete()
    {
        ...
        if ($this->request->getPost('product_id')) {
            ...
            $update = $productModel->where('seller_id', session()->get('user_id'))->update($productId, ['is_delete' => 1]);
    ...
    public function add()
    {
        ...
        $findIsCategoryActiveForSeller = $sellerCategoriesModel->findIsCategoryActiveForSeller($categoryname);

        $sellerModel = new SellerModel();
        $findSellerInfo = $sellerModel->select('require_products_approval')->where('id', $seller)->first();
        ...
        if (isset($findIsCategoryActiveForSeller['id'])) {
            ...
            if ($appSetting['seller_approval_product'] == 1 && $findSellerInfo['require_products_approval'] == 0 ) {
                $productData = [
                    ...
                    'added_by_seller' => 1,
                    'status' => filter_var($this->request->getPost('ispublish'), FILTER_VALIDATE_INT),
                    'seo_title' => $seo_title,
            ...
            if ($productid) {
                ...
                if ($ptype == "simple_product") {
                    ...
                    $productVariantsModel->insert($variantData);
                } else {
    ...
    public function edit($id)
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            $images = $productImagesModel
                ->where('product_id', $id)
                ->findAll();
            return view('sellerPanel/product/edit', [
                'categories' => $categories,
    ...
    public function update()
    {
        ...
        $settingModel = new SettingsModel();
        $appSetting = $settingModel->getSettings();
        if ($appSetting['seller_approval_product'] == 1) {
            $productData = [
                'product_name' => $productname,
                'brand_id' => $brandname,
                'category_id' => $categoryname,
        ...
        if ($this->request->getPost('ispublish') != 1) {
            $cartsModel = new CartsModel();
            $cartsModel->where('product_id', $product_id)->delete();
        }
    ...
    public function bulkImport()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            return view('sellerPanel/product/bulkImport', [
                'settings' => $appSetting,
            ]);
        } else {
            return redirect()->to('seller/auth/login');
    ...
    public function bulkImportFile()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            if (!empty($data)) {
                ...
                session()->setFlashdata('error', 'No data found in CSV.');
                return redirect()->back();
            }
        }
    ...
    public function exportProductsInCSV()
    {
        ...
        $products = $productModel->where('is_delete', 0)->where('seller_id', session()->get('user_id'))->findAll();
    ...
    public function bulkUpdate()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            return view('sellerPanel/product/bulkUpdate', [
                'settings' => $appSetting,
            ]);
        } else {
            return redirect()->to('seller/auth/login');
    ...
    public function bulkUpdateFile()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            while (($row = fgetcsv($csvFile, 1000, ',')) !== false) {
                ...
                // Update variants
                $variantStart = 17; // Variant data starts after tags
    ...
    public function productOrder()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            return view('sellerPanel/product/productOrder', [
                'settings' => $appSetting,
                'categories' => $categories
            ]);
        } else {
    ...
    public function viewProduct($productId)
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            $product = $productModel->where('seller_id', session()->get('user_id'))->find($productId);
            ...
            $seller = $sellerModel->find(session()->get('user_id'));
            $tags = $productTagModel
                ->select('tags.name')
            ...
            return view('sellerPanel/product/viewProduct', [
                'settings' => $settingModel->getSettings(),
                'product' => $product,
                'category' => $category,
    ...
    public function deleteOtherImage()
    {
        ...
        $productImagesModel = new ProductImagesModel();
        $image = $productImagesModel->select('product_images.id')
            ->where('product_images.product_id', $productId)
            ->join('product', 'product.id = product_images.product_id', 'left')
            ->where('product_images.id', $id)
            ->where('product.seller_id', session()->get('user_id'))
            ->first();
        if ($image) {
            $productImagesModel->delete($id);
